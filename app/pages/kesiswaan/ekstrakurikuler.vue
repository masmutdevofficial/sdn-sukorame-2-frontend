...
<template>
  <div>
    ...
    <section class="container-school py-12"><p class="mb-5 text-sm text-muted" aria-live="polite">{{ filteredItems.length }} kegiatan ditemukan</p><div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3"><article v-for="item in visibleItems" :key="item.id" class="card overflow-hidden"><div class="flex snap-x overflow-x-auto"><img v-for="(image,index) in item.images" :key="index" :src="image" :alt="item.title" class="aspect-[4/3] w-full shrink-0 snap-center object-cover"><img v-if="!item.images.length" src="https://cdn.sdnsukorame2kotakediri.sch.id/images/no-image.png" alt="Belum ada gambar" class="aspect-[4/3] w-full object-contain"></div><div class="p-5"><p class="text-xs text-muted">{{ new Date(item.date).toLocaleDateString('id-ID',{dateStyle:'long',timeZone:'Asia/Jakarta'}) }}</p><h2 class="mt-2 text-xl font-bold text-school-navy">{{ item.title }}</h2><p class="mt-3 text-sm leading-6 text-muted">{{ item.description }}</p></div></article></div><p v-if="!visibleItems.length" class="card mt-6 p-8 text-center text-muted">Tidak ada kegiatan yang sesuai dengan pencarian.</p><nav v-if="totalPages > 1" class="mt-9 flex flex-wrap items-center justify-center gap-2" aria-label="Pagination kegiatan"><button class="btn btn-secondary !px-4 !py-2.5" :disabled="page===1" @click="page--"><Icon name="mdi:chevron-left"/> Sebelumnya</button><button v-for="number in totalPages" :key="number" class="grid size-11 place-items-center rounded-xl border font-bold" :class="number===page?'border-school-action bg-school-action text-white':'border-line bg-white text-school-navy'" :aria-current="number===page?'page':undefined" @click="page=number">{{ number }}</button><button class="btn btn-secondary !px-4 !py-2.5" :disabled="page===totalPages" @click="page++">Berikutnya <Icon name="mdi:chevron-right"/></button></nav></section>
  </div>
</template>
