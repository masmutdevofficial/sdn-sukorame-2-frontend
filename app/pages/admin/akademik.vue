...
<template>
  <div>
    ...
    <AdminModal :open="editing!==null" :title="editing===-1?'Tambah Dokumen':'Edit Dokumen'" @close="editing=null"><form class="form-grid" @submit.prevent="store"><label class="field">Judul<input v-model="form.title" required></label><label class="field">Status<select v-model="form.status"><option>draft</option><option>published</option></select></label><label class="field md:col-span-2">Deskripsi<textarea v-model="form.description" rows="4"/></label><div class="md:col-span-2"><AdminPdfUpload v-model="form.file" v-model:filename="form.filename"/></div><div class="md:col-span-2 flex justify-end gap-2"><button type="button" class="btn btn-secondary" @click="editing=null">Batal</button><button type="submit" class="btn btn-primary" :disabled="!form.file">Simpan</button></div></form></AdminModal>
  </div>
</template>
