...
const { data: pageData } = await useAsyncData('home-page', () => homePageRepository.get('beranda'))
const page = computed(() => pageData.value!)
const { data: programs } = await useAsyncData('home-programs', async () => (await contentRepository.list({ module: 'program', status: 'published', perPage: 100 })).items, { default: () => [] })
    ...
    <section class="relative min-h-[690px] overflow-hidden bg-[#f4faff]">
      ...
      <div class="container-school relative grid min-h-[690px] items-center gap-14 py-16 lg:grid-cols-[1.04fr_.96fr] lg:py-20">
        ...
        <div class="hero-visual relative mx-auto w-full max-w-[560px] lg:mx-0">
          ...
          <div class="float-slow absolute -left-5 top-14 z-20 hidden rounded-2xl border border-white bg-white px-4 py-3 shadow-xl sm:-left-12 sm:block"><div class="flex items-center gap-3"><span class="grid size-10 place-items-center rounded-xl bg-emerald-100 text-emerald-700"><Icon name="mdi:leaf" size="22" /></span><span><b class="block text-sm text-school-navy">{{ page.hero.leftBadgeTitle }}</b><small class="text-muted">{{ page.hero.leftBadgeDescription }}</small></span></div></div>
          <div class="float-reverse absolute -right-3 bottom-24 z-20 hidden rounded-2xl border border-white bg-white px-4 py-3 shadow-xl sm:-right-10 sm:block"><div class="flex items-center gap-3"><span class="grid size-10 place-items-center rounded-xl bg-amber-100 text-amber-700"><Icon name="mdi:lightbulb-on-outline" size="22" /></span><span><b class="block text-sm text-school-navy">{{ page.hero.rightBadgeTitle }}</b><small class="text-muted">{{ page.hero.rightBadgeDescription }}</small></span></div></div>
        </div>
      </div>
    </section>
      ...
      <div class="container-school relative grid items-center gap-12 lg:grid-cols-[.8fr_1.2fr]">
        ...
        <div class="grid gap-4 sm:grid-cols-2">
          <article v-for="(program,index) in programs.slice(0,page.programs.itemLimit)" :key="program.id" class="interactive-card reveal card p-6" :class="index===1||index===2?'sm:translate-y-6':''">
            <span class="icon-bubble grid size-12 place-items-center rounded-2xl bg-school-sky text-school-action"><Icon :name="program.icon || 'mdi:school-outline'" size="27" /></span><h3 class="mt-5 text-lg font-bold text-school-navy">{{program.title}}</h3><p class="mt-2 text-sm leading-6 text-muted">{{program.excerpt}}</p>
          </article>
        </div>
      </div>
    </section>
