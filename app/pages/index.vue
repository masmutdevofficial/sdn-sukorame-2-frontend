...
import { school, news, programs } from '~/data/dummy/content'
useSchoolSeo('Beranda', school.tagline)
...
const welcomeMessages=[
  'Selamat datang di website resmi SD Negeri Sukorame 2 Kota Kediri',
  'Bersama menumbuhkan generasi yang berakhlak, kritis, terampil, dan inovatif',
  'Sekolah SAKTI — tempat belajar, bertumbuh, dan berkarya dengan gembira',
]
</script>
...
<template>
  <div class="overflow-hidden">
      ...
      <div class="container-school relative grid min-h-[690px] items-center gap-14 py-16 lg:grid-cols-[1.04fr_.96fr] lg:py-20">
        <div class="relative z-10">
          <div class="hero-enter inline-flex items-center gap-2 rounded-full border border-school-blue/15 bg-white/90 px-4 py-2 text-sm font-bold text-school-action shadow-sm">
            ...
            Sekolah dasar negeri di Kota Kediri
          </div>
          <h1 class="hero-enter-delay font-display mt-7 max-w-3xl text-[2.8rem] leading-[1.04] tracking-[-.02em] text-school-navy sm:text-6xl lg:text-[4.35rem]">
            Tempat anak tumbuh<br><span class="relative text-school-action">menjadi hebat.<svg class="absolute -bottom-2 left-0 w-full text-school-yellow" viewBox="0 0 300 12" fill="none" aria-hidden="true"><path d="M3 8C73 2 190 2 297 7" stroke="currentColor" stroke-width="6" stroke-linecap="round"/></svg></span>
          </h1>
          <p class="hero-enter-delay mt-8 max-w-xl text-base leading-8 text-slate-600 sm:text-lg">Pembelajaran yang dekat dengan alam, berakar pada karakter, dan membuka ruang bagi setiap anak untuk berpikir, berkarya, serta percaya diri.</p>
          <div class="hero-enter-delay mt-9 flex flex-wrap gap-3">
            <NuxtLink to="/profil" class="btn btn-primary">Jelajahi sekolah <Icon name="mdi:arrow-right" /></NuxtLink>
            <NuxtLink to="/informasi/berita" class="btn btn-secondary"><Icon name="mdi:play-circle-outline" /> Lihat kegiatan</NuxtLink>
          </div>
          <div class="hero-enter-delay mt-10 flex flex-wrap gap-x-8 gap-y-4 border-t border-school-navy/10 pt-6">
            <div><b class="font-display text-2xl text-school-navy">1957</b><p class="text-xs text-muted">Tahun berdiri</p></div>
            <div><b class="font-display text-2xl text-school-navy">SAKTI</b><p class="text-xs text-muted">Nilai utama</p></div>
            <div><b class="font-display text-2xl text-school-navy">Kediri</b><p class="text-xs text-muted">Kota pendidikan</p></div>
          </div>
        </div>
        ...
        <div class="hero-visual relative mx-auto w-full max-w-[520px] lg:mx-0">
          ...
          <div class="float-slow absolute -left-5 top-14 rounded-2xl border border-white bg-white px-4 py-3 shadow-xl sm:-left-12"><div class="flex items-center gap-3"><span class="grid size-10 place-items-center rounded-xl bg-emerald-100 text-emerald-700"><Icon name="mdi:leaf" size="22" /></span><span><b class="block text-sm text-school-navy">Ramah lingkungan</b><small class="text-muted">Belajar dari alam</small></span></div></div>
          <div class="float-reverse absolute -right-3 bottom-24 rounded-2xl border border-white bg-white px-4 py-3 shadow-xl sm:-right-10"><div class="flex items-center gap-3"><span class="grid size-10 place-items-center rounded-xl bg-amber-100 text-amber-700"><Icon name="mdi:lightbulb-on-outline" size="22" /></span><span><b class="block text-sm text-school-navy">Kreatif & inovatif</b><small class="text-muted">Berani berkarya</small></span></div></div>
        </div>
      </div>
    </section>
    ...
    <section class="container-school py-20 lg:py-28">
      <div class="reveal mx-auto max-w-2xl text-center"><p class="font-hand text-3xl font-bold text-school-action">Karakter yang kami tumbuhkan</p><h2 class="font-display mt-2 text-4xl text-school-navy sm:text-5xl">Lima nilai, satu semangat SAKTI</h2><p class="mt-5 leading-7 text-muted">Nilai bukan sekadar kata. Ia hadir dalam cara belajar, bermain, bekerja sama, dan merawat lingkungan setiap hari.</p></div>
      <div class="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        <article v-for="(value,index) in values" :key="value.letter" class="interactive-card reveal card relative overflow-hidden p-6" :style="`animation-range:entry ${index*2}% cover ${24+index*2}%`">
    ...
    <section class="relative bg-white py-20 lg:py-28">
      ...
      <div class="container-school relative grid items-center gap-12 lg:grid-cols-[.8fr_1.2fr]">
        <div class="reveal"><p class="font-hand text-3xl font-bold text-school-action">Belajar dengan gembira</p><h2 class="font-display mt-2 text-4xl leading-tight text-school-navy sm:text-5xl">Program yang dekat dengan kehidupan.</h2><p class="mt-5 leading-7 text-muted">Setiap program dirancang untuk memberi pengalaman nyata: mengamati, mencoba, berdiskusi, lalu menciptakan sesuatu yang bermakna.</p><NuxtLink to="/akademik/program-sekolah" class="mt-7 inline-flex items-center gap-2 font-bold text-school-action">Lihat semua program <Icon name="mdi:arrow-right" /></NuxtLink></div>
        <div class="grid gap-4 sm:grid-cols-2">
          <article v-for="(program,index) in programs.slice(0,4)" :key="program.id" class="interactive-card reveal card p-6" :class="index===1||index===2?'sm:translate-y-6':''">
    ...
    <section class="container-school py-20 lg:py-28">
      <div class="reveal flex flex-wrap items-end justify-between gap-5"><div><p class="font-hand text-3xl font-bold text-school-action">Cerita dari sekolah</p><h2 class="font-display mt-1 text-4xl text-school-navy sm:text-5xl">Kabar terbaru</h2></div><NuxtLink to="/informasi/berita" class="btn btn-secondary">Semua berita <Icon name="mdi:arrow-right" /></NuxtLink></div>
      <div class="mt-10 grid gap-6 md:grid-cols-3">
        <article v-for="(item,index) in news.slice(0,3)" :key="item.id" class="interactive-card reveal card group overflow-hidden" :style="`animation-range:entry ${index*3}% cover ${25+index*3}%`">
          ...
          <div class="p-6"><p class="text-xs font-semibold text-muted">{{new Date(item.date).toLocaleDateString('id-ID',{day:'numeric',month:'long',year:'numeric'})}}</p><h3 class="mt-3 text-lg font-bold leading-7 text-school-navy">{{item.title}}</h3><p class="mt-3 line-clamp-2 text-sm leading-6 text-muted">{{item.excerpt}}</p><NuxtLink :to="`/informasi/berita/${item.slug}`" class="mt-5 inline-flex items-center gap-2 text-sm font-bold text-school-action">Baca cerita <Icon name="mdi:arrow-right" /></NuxtLink></div>
        </article>
      </div>
    </section>
    ...
    <section class="container-school pb-20 lg:pb-28">
      <div class="reveal relative overflow-hidden rounded-[2.2rem] bg-school-navy px-6 py-14 text-center text-white sm:px-12 lg:py-16">
        ...
        <div class="relative mx-auto max-w-2xl"><p class="font-hand text-3xl font-bold text-school-yellow">Mari bertumbuh bersama</p><h2 class="font-display mt-2 text-4xl sm:text-5xl">Kenali lebih dekat Sekolah SAKTI.</h2><p class="mt-5 leading-7 text-blue-100">Temukan profil, program, kegiatan, dan informasi sekolah melalui website resmi SD Negeri Sukorame 2 Kota Kediri.</p><div class="mt-8 flex flex-wrap justify-center gap-3"><NuxtLink to="/profil" class="btn bg-school-yellow text-school-navy hover:shadow-xl">Tentang sekolah</NuxtLink><NuxtLink to="/kontak" class="btn border border-white/30 text-white hover:bg-white/10">Hubungi kami</NuxtLink></div></div>
      </div>
    </section>
  </div>
</template>
